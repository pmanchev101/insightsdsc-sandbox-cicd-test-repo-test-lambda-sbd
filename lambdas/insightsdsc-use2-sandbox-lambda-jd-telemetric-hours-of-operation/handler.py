...
def lambda_handler(event, context):
    ...
    daily_hours_of_operation = []
    for jd_machine_id in machine_ids:
        machine_hours_of_operation_link = f'https://sandboxapi.deere.com/platform/machines/{jd_machine_id}/hoursOfOperation' + ';count=100'
        machine_hours_of_operation = get_with_paginate_helper(oauth2_session, machine_hours_of_operation_link,
                                                       MYJOHNDEERE_V3_JSON_HEADERS, params)
        for machine_hour_of_operation in machine_hours_of_operation:
            new_hour_of_operation_dict = {'machine_hours_of_operation_machine_id': jd_machine_id,
                                          'machine_hours_of_operation_start_date': machine_hour_of_operation[
                                              'startDate'],
                                          'machine_hours_of_operation_end_date': machine_hour_of_operation['endDate'],
                                          'machine_hours_of_operation_engine_state': machine_hour_of_operation[
                                              'engineState']}
            daily_hours_of_operation.append(new_hour_of_operation_dict)
        time.sleep(0.1)

    csv_header = ['machine_hours_of_operation_machine_id', 'machine_hours_of_operation_start_date',
                  'machine_hours_of_operation_end_date', 'machine_hours_of_operation_engine_state']

    csv_file_name = 'hours_of_operation.csv'

    with open('/tmp/' + csv_file_name, 'w', newline='') as csvfile:
        csvwriter = csv.DictWriter(csvfile, fieldnames=csv_header)
        csvwriter.writeheader()
        for item in daily_hours_of_operation:
    ...
    s3c.upload_file('/tmp/' + csv_file_name, bucket_name, "data/john-deere/hours-of-operation/date={date}/".format(
        date=start_date.strftime("%Y%m%d")) + csv_file_name)

    return {
        'message': "Uploaded {hours_of_operation} engine hours for {machines} machines to S3 for date {date}".format(
            hours_of_operation=len(daily_hours_of_operation), machines=len(machine_ids), date=start_date.strftime("%Y-%m-%d"))
    }
